...
void reflow_terminal_text_old(struct term_context *ctx, struct pos old_term_size, struct term_cell **temp_term_cell_ptr) {
    // OLD LOGIC
    int term_cell_alloc_size = ctx->term_size.w * ctx->term_size.h;
    ...
    int old_cursor_y = ctx->cur->cur_pos.h;
    int target_cursor_y = old_cursor_y < term_size.h ? old_cursor_y : term_size.h - 1;
    int start_line = new_cursor_h - target_cursor_y;
...
int main() {
    ...
    const char *text = "1234567890"; // exactly 1 line
    int len = strlen(text);
    int y=0, x=0;
    for(int i=0; i<len; i++) {
    ...
    ctx.cur->cur_pos.w = x; // 0
    ctx.cur->cur_pos.h = y; // 1
    
    print_term(&ctx);
    
    printf("\n--- OLD LOGIC: Resizing to 5x5 ---\n");
    ...
    reflow_terminal_text_old(&ctx, old_size2, &temp2);
    print_term(&ctx);
